...
- `RP_CONFIG_USE_ROTOM_PHONE`: Rotom Phone or generic flip phone.
- `RP_CONFIG_PHONE_COLOUR`: Which of the 16 palettes to use.
- `RP_CONFIG_MONOCHROME_ICONS`: Whether icons use their individual colours or are monochrome.
- `RP_CONFIG_PALETTE_BUFFER`: Cache background and sprite palettes in EWRAM, recommended if using coloured icons or changing the monochrome sprite colour in code.*
- `RP_CONFIG_ROTOM_REALITY_SHORTCUT`: Whether START opens the starred shortcut option in Rotom Reality.
- `RP_CONFIG_24_HOUR_MODE`: 24-hour or 12-hour clock display.
- `RP_CONFIG_NUM_MINUTES_TO_UPDATE`: In-game minutes between time and Rotom message updates.
- `RP_CONFIG_UPDATE_MESSAGE`: Whether Rotom delivers messages on the overworld timer.
- `RP_CONFIG_UPDATE_MESSAGE_SOUND`: Whether a sound plays with each Rotom message.
- `RP_CONFIG_FACE_UPDATE_PERCENT`: Percentage chance Rotom's expression changes on a message or update.
