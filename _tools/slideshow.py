...
def _draw_indicator(draw: ImageDraw.ImageDraw, slide_num: int, total: int, dot_color: str, width: int, height: int, position: str = "bottom-right") -> None:
    ...
    for i in range(total):
        ...
        if i == slide_num - 1:
            ...
            draw.ellipse(bbox, outline=dot_color, width=2)
...
def main():
    ...
    for i, slide in enumerate(slides, 1):
        ...
        dot_color_raw = slide.get("dotColor", default_dot_color)
        dot_color = _resolve_color(dot_color_raw) if dot_color_raw else fg
